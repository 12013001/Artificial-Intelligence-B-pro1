function [hist] = Test_characters_temp(Teststruct)
%TEST_HIST 此处显示有关此函数的摘要
%   此处显示详细说明
% hist = struct('r',[],'g',[],'b',[],'h',[],'s',[],'v',[],'l',[],'a',[],'b1',[],'texture',[]);
hist = struct('h',[],'texture',[]);
hist(1) = [];
hist = repmat(hist,[1,length(Teststruct)]);
for i = 1:length(Teststruct)
%     [r,g,b] = imsplit(Teststruct(i).data);
    [h,s,v] = imsplit(rgb2hsv(Teststruct(i).data));
%     [l,a,b1] = imsplit(rgb2lab(Teststruct(i).data));
...
%     temp(:,:,1) = imresize(Teststruct(i).data(:,:,1),[100,100]);
%     temp(:,:,2) = imresize(Teststruct(i).data(:,:,2),[100,100]);
%     temp(:,:,3) = imresize(Teststruct(i).data(:,:,3),[100,100]);
%     feature = double(extractHOGFeatures(uint8(temp)))';
%     hist(i).texture = feature/sum(feature(:));
    
    %此处等待装载texture和structure的操作
    
    temp = rgb2gray(Teststruct(i).data);
