function output = C_partA(target,pixel,tile,repeat,is_man)
...
if is_man == 1
    loader_hist = load("pA_features.mat","man_loader_hist");
    loader_hist = loader_hist(1).man_loader_hist;
    loader_all = load("pA_features.mat","man");
    loader_all = loader_all(1).man;
else
    loader_hist = load("pA_features.mat","na_loader_hist");
    loader_hist = loader_hist(1).na_loader_hist;
    loader_all = load("pA_features.mat","na");
    loader_all = loader_all(1).na;
end
...
%loader_all = C_dataLoader([delta_h,delta_w],file);%,'.\dataset\manmade_test\*.jpg','.\dataset\natural_test\*.jpg');
% 以上为根据大小加载的数据加载器
...
for i = 1:length(target_sub)
    [a4,~,~] = imsplit(rgb2hsv(target_sub(i).data));
...
for j = 1:tile
    ...
    for i = 1:length(loader_hist)
        if loader_all(i).repeat < repeat    
            temp_dist = C_pA_distance(loader_hist(i),target_hist(j)); %注意select的填充顺序与target_sub一致，先上-下，再左-右
        ...
        if i == length(loader_hist) % 用来更新
            select(j).data =imresize(loader_all(record).data,[delta_h, delta_w],'nearest');%在这里载入相应图片来resize
...
% output_1=filter2(A,output(:,:,1),'same');
% output_2=filter2(A,output(:,:,2),'same');% 高斯滤波
% output_3=filter2(A,output(:,:,3),'same');
output = C_gaussian(output);
output_1=output(:,:,1);
output_2=output(:,:,2);% 高斯滤波
output_3=output(:,:,3);
output_1 = medfilt2(output_1,[3,3]);
output_2 = medfilt2(output_2,[3,3]);% 中值滤波
output_3 = medfilt2(output_3,[3,3]);
